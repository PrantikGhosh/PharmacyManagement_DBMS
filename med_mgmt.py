...
def make_bill(): # makes bill
    ...
    m+="  SRM University, Kattankulathur, Chennai, T.N.\n\n"
    m+="-----------------------------------------------\n"
    if t==1:
        m+="Name: %s\n" % named
        m+="Address: %s\n" % addd
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    if flag=='apt':
        apt.destroy()
    root=Tk()
    root.title('DBMS-Pharmacy Management')
    Label(root,text='CHEMIST AND DRUG SHOP').grid(row=0,column=0,columnspan=5)
    Label(root,text="SRM UNIVERSITY, KATTANKULATHUR, Chennai,  TN").grid(row=1,column=0,columnspan=5)
